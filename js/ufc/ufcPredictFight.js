async function predictFight(f1, f2, advantages, roundSimulation) {
    const userMessage = `(using "###" to represent new paragraph) Estimate the outcome and list the stats according to betting odds.  Use the following information:${f1.name}, ${f2.name}. Stat Comparison: ${advantages}.  Round simulation: ${roundSimulation} (start by clearly announcing the decided winner) `;
    ...
    let aiPrompt = `(using "###" to represent new paragraph) Predict the outcome if these two fighters fought, include how the statistics would look and include betting odds. limit to around 1000 words or less: ${f1.name} vs. ${f2.name}.(start by clearly announcing your decided winner)`;
